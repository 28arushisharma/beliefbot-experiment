...
doc = """
Stage 5 — Writer chooses which sample to show Reader.

12 rounds: 4 matches × 3 sub-rounds each.
Two jars: jar 1 (rounds 1-6, matches 1-2), jar 2 (rounds 7-12, matches 3-4, opposite colour).

Player 1 = Writer: sees all 20 balls + 6 candidate samples (drawn WITH replacement); selects
          one sample to send to Reader; knows true jar.
...
most strongly points toward the WRONG jar.
...
Payoffs per sub-round:
  ...
  Reader's net payoff = base payoff − additional draw cost (matches 3-4 only).
"""
...
PAYOFF_ONE_WRONG    = 0
DRAW_COST_PER_BALL  = 2.0  # matches 3-4 only
...
def _seed_jar(session_code):
    return abs(hash((session_code, 'stage5_jar'))) % (2 ** 31)

def _seed_samples(session_code, round_number):
    return abs(hash((session_code, 'stage5_samples', round_number))) % (2 ** 31)

def _seed_additional(session_code, round_number):
    return abs(hash((session_code, 'stage5_add', round_number))) % (2 ** 31)
...
class ReaderPage(Page):
    ...
    def vars_for_template(player):
        ...
        return dict(
            **_instructions_vars(),
            ...
            round_in_match=_round_in_match(player.round_number),
            is_paid_draws=is_paid,
            draw_cost_per_ball=int(C.DRAW_COST_PER_BALL) if is_paid else 0,
            is_paid_draws_js='true' if is_paid else 'false',
            cumulative_earnings=int(player.participant.vars.get('cumulative_earnings', 0)),
        )
    ...
    def before_next_page(player, timeout_happened):
        ...
        match_num = _match_number(player.round_number)
        cost_per  = C.DRAW_COST_PER_BALL if match_num > C.BOT_MATCHES else 0.0
        player.additional_draw_cost = n_add * cost_per
...
class ResultsWaitPage(WaitPage):
    ...
    def after_all_players_arrive(group):
        ...
        if not p1.guess or not p2.guess:
            raise ValueError(
                f"Stage 5 round {group.round_number}: missing guess "
